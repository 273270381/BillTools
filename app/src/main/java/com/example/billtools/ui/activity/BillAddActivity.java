...
import com.example.billtools.presenter.BillPresenter;
import com.example.billtools.presenter.contract.BillContract;
import com.example.billtools.ui.adapter.BookNoteAdapter;
import com.example.billtools.ui.adapter.MonthAccountAdapter;
import com.example.billtools.utils.DateUtils;
import com.example.billtools.utils.ProgressUtils;
...
import static com.example.billtools.utils.DateUtils.FORMAT_D;
import static com.example.billtools.utils.DateUtils.FORMAT_M;
import static com.example.billtools.utils.DateUtils.FORMAT_Y;
import static com.example.billtools.utils.DateUtils.FORMAT_YMD;
...
public class BillAddActivity extends BaseMVPActivity<BillContract.Presenter>
    ...
    protected void initData(Bundle savedInstanceState) {
        ...
        coBill = (CoBill) getIntent().getSerializableExtra("cobill");

        if (coBill != null) {
            isEdit = true;
            //设置账单日期
            days = DateUtils.long2Str(coBill.getCrdate(), FORMAT_YMD);
            isOutcome = !coBill.getIncome();
            remarkInput = coBill.getContent();
            DecimalFormat df = new DecimalFormat("######0.00");
            String money = df.format(coBill.getCost());
            //小数取整
            num = money.split("\\.")[0];
            //截取小数部分
...
//                    DateUtils.getMillis(crDate), !isOutcome, bundle.getInt("version") + 1);
            mPresenter.updateBill(coBill);
        } else {
//            coBill = new CoBill(null, null, Float.valueOf(num + dotNum), remarkInput,
...
//                    DateUtils.getMillis(crDate), !isOutcome, 0);
            ...
            coBill = new CoBill(Float.valueOf(num + dotNum), remarkInput,MyApplication.getCurrentUserId(),
                    noteBean.getPayinfo().get(selectedPayinfoIndex).getPayName(),
                    noteBean.getPayinfo().get(selectedPayinfoIndex).getPayImg(),
                    lastBean.getSortName(),lastBean.getSortImg(),DateUtils.getMillis(crDate),!isOutcome, 0);
            mPresenter.addBill(coBill);
        }
    }
            ...
            public void onPageSelected(int position) {
                try {
                    for (int i = 0; i < viewList.size(); i++) {
                        icons[i].setImageResource(R.mipmap.icon_banner_point2);
                    }
                    icons[position].setImageResource(R.mipmap.icon_banner_point1);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    protected void initIcon() {
        ...
        for (int i = 0; i < icons.length; i++) {
            ...
            if (viewpagerItem.getCurrentItem() == i) {
                icons[i].setImageResource(R.mipmap.icon_banner_point1);
            }
